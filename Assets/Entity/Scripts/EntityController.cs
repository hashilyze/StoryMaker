...
namespace Stroy {
    namespace Entity {
        ...
        public sealed class EntityController : MonoBehaviour {
            ...
            public Rigidbody2D FollowBlock { get => m_followPlatform; set => SetFollower(value); }
            ...
            // Follow platform
            public void SetFollower(Rigidbody2D followPlatform) {
            ...
            private void OnCollisionEnter2D(Collision2D collision) {
                if (collision.gameObject.layer == PlatformConstants.L_DynamicBlock && collision.collider.isTrigger == false) {
                    ++m_dynamicBlockNum;
                }
            }
            ...
            private bool ReactBlock(Vector2 origin, out Vector2 preDistance, out Vector2 postDistance) {
                            ...
                            if (detectedCollider.gameObject.layer == PlatformConstants.L_DynamicBlock) {
                                ...
                                if (Vector2.Dot(newPushVelocity, newPenetration) > 0f) { // Query only forward objects
                                    ...
                                    if (colliderDist.normal.x != 0f && Mathf.Abs(maxPushVelocity.x) < Mathf.Abs(newPushVelocity.x)) {
                                        pushedByFollowX = m_existFollow && detectedCollider.attachedRigidbody == m_followPlatform;
                                        maxPushVelocity.x = newPushVelocity.x;
                                    }
                                    if (colliderDist.normal.y != 0f && Mathf.Abs(maxPushVelocity.y) < Mathf.Abs(newPushVelocity.y)) {
                                        pushedByFollowY = m_existFollow && detectedCollider.attachedRigidbody == m_followPlatform;
                ...
                {
                    if (m_existFollow) {
                        followDistance = m_getFollowDistance(m_followPlatform);
                    } else {
            ...
            private void AddDistance(ref Vector2 destination, float distance, Vector2 axis) {
                ...
                for (int n = 0; n != numBlock; ++n) {
                    ...
                    if (blockHit.collider.gameObject.layer == PlatformConstants.L_DynamicBlock) {
...
#if UNITY_EDITOR
            #region DEBUG
            private Vector2 editor_center;
            private Vector2 editor_size;
            private Vector2 editor_tr;
            private Vector2 editor_br;
            private Vector2 editor_tl;
            private Vector2 editor_bl;
